...
There is only one bank. This bank has an array of accounts. The bank needs a
 method that will return the total sum of money in the accounts. It also needs an addAccount method
that will enroll a new account at the bank and add it to the array of accounts.
...
that the methods do indeed
work as expected: add some accounts, show the total balance, make some deposits
...
var bank = {
  accounts: [
    { name: "Pat", currentBal: 40 },
    { name: "Bob", currentBal: 200 }
  ],
  ...
  totalSum: function() {
    ...
    for(var i=0; i<this.accounts.length; i++){
      ...
      tot += this.accounts[i].currentBal;
    }
    console.log("Total bank balance: "+tot);
  },
  addAccount: function(name, bal) {
    this.accounts.push({name:"Kira", currentBal:10} );

  },
  withdraw: function(name, amount){
    for(var i=0;i<this.accounts.length;i++){
      if(this.accounts[i].name===name){
        this.accounts[i].currentBal-=amount;
      }
...
this.withdraw(name1, amount);
this.deposit(name2, amount);
      }*/


//(bank.transfer("kira","Pat", 10);
console.log(bank.deposit("Pat",30));
bank.withdraw("Bob", 20);
bank.addAccount("Kira", 10);
bank.totalSum();
