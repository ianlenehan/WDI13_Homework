...
var withdraw = function() {
  ...
  var withdrawAmount = +($withdrawInput.val());
  var currentBalance = +($balanceWindow.html().slice(1));
  var newBalance = currentBalance - withdrawAmount;
  if (newBalance < 0) {
    if ( +($('#savings-balance').html().slice(1)) + +($('#checking-balance').html().slice(1)) >= withdrawAmount ) {
      ...
      var otherBalance = +($otherAccount.html().slice(1));
...
var deposit = function() {
  ...
  var withdrawAmount = +($depositInput.val());
  var currentBalance = +($balanceWindow.html().slice(1));
  var newBalance = currentBalance + withdrawAmount;
  $balanceWindow.html('$' + newBalance);
  $depositInput.val('');
